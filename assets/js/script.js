$(function(){
    ...
    $.getJSON('http://api.flickr.com/services/rest/?&method=flickr.photosets.getPhotos&api_key=' + apiKey + '&photoset_id=72157619415192530&format=json&jsoncallback=?',function(res){
        //loop through the results with the following function
        $.each(res.photoset.photo, function(i,item){
            data.push({
                'thumb' : 'http://farm'+ item.farm +'.staticflickr.com/'+ item.server +'/'+ item.id +'_'+ item.secret +'_q.jpg'
            });
        });
    ...
    gallery.on('data-ready window-resized page-turned', function(event, direction){
        ...
        console.log(get_page_start() + get_per_page());
